...
public sealed class RenderSessionApiTests
{
    ...
    private sealed class ApiFixture : IAsyncDisposable
    {
        ...
        private ApiFixture(WebApplication app, RenderSessionManager manager, HttpClient client)
        {
            _app = app;
            _manager = manager;
            Client = client;
        ...
        public static async Task<ApiFixture> StartAsync(int cap)
        {
            ...
            return new ApiFixture(app, manager, new HttpClient { BaseAddress = new Uri(address) });
        }
