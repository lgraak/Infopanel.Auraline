...
This document records the architecture for InfoPanel.Auraline. M1 implemented the Windows tray Host and provider foundation, M2 added the Host-owned waveform engine, M3 added render sessions plus Windows local frame transport, M4 implemented the thin Windows InfoPanel consumer, M5 implemented persistent product configuration plus the loopback management UI, and M6 adds local diagnostics and manual beta packaging. M4 direct runtime acceptance passed against the matching local Windows prerequisite; repository implementation, local activation, package generation, and public InfoPanel availability remain separate states.
...
M6 keeps `/health` concise and adds richer loopback-only `/api/v1/diagnostics`, summary, self-test, log-level, and export routes. Diagnostics aggregate bounded current-run state already owned by providers, waveform, configuration, sessions, and the plugin; they do not add persistent metrics or polling. The self-test allocates its own temporary render transport, publishes and reads a synthetic rendered frame, and disposes it without joining active consumer sessions or mutating configuration. Export is local and user-initiated, applies deterministic redaction, includes at most seven recent 10 MiB log files, and excludes raw audio, waveform samples, and frame pixels.

RB1B adds bounded current-run stall observability without changing scheduler, renderer, transport, provider, retry, or logging behavior. Session cadence calculations remain unchanged; separate `Stopwatch`-based monotonic timestamps measure scheduled deadline, actual render start, scheduler lateness, renderer duration, synchronous transport-publication duration, and publication interval. Per-session summaries retain current/max values and threshold counters, while only significant timing and lifecycle events enter a 32-item in-memory ring. Supported .NET in-process GC APIs expose collection counts, managed/heap/committed bytes, GC index/generation, and pause duration without profiler or EventPipe attachment. Wall-clock timestamps label significant events for human correlation but never participate in cadence math. The loopback diagnostics API/UI and user-initiated export expose this observational state; nothing is automatically persisted.

The manual beta package is framework-dependent Windows x64. Platform-neutral diagnostic models stay in ordinary Host code; only the existing Windows frame-transport implementation and the PowerShell packaging target are Windows-specific. Host and plugin remain architecturally separate inside the combined ZIP, and the plugin retains the exact four-file M4 package boundary.
...
RB1B instrumentation observes that existing loop exactly where work already occurs. It does not select deadlines, alter missed-frame policy, introduce another timer, log per-frame data, or retain frame history. Render duration remains independently visible from scheduler lateness and transport publication so a future visible stall can be classified without treating aggregate FPS as cadence evidence.
