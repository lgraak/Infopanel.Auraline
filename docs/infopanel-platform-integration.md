...
Direct M4 acceptance on 2026-08-25 used that exact local prerequisite build. InfoPanel loaded both outputs, supplied `600x150` and `300x300` demands, displayed Active and Idle Host-rendered frames with the expected transparency and color, released both leases on plugin unload, restored them on reload, showed the explicit unavailable surface during a controlled Host outage, and recovered both new sessions without restarting InfoPanel. Direct InfoPanel mapping measurements observed about 27–28.5 publishes per second for simultaneous 30 FPS sessions and about 48–51.5 for simultaneous 60 FPS sessions while Host sessions ran about 29.8 and 57.6 FPS. The 60 FPS result is therefore a bounded sanity check, not full 60 FPS display acceptance. The dated M4 handoff separates this local runtime proof from public-build availability.
